...
/* 0xff fill after the SOI rounds the injected tables up to the PRD alignment */
#define WMT_JDEC_FILL_LEN	(WMT_JDEC_PRD_ALIGN - WMT_JDEC_DHT_LEN % WMT_JDEC_PRD_ALIGN)
#define WMT_JDEC_PRE_LEN	(2 + WMT_JDEC_FILL_LEN)

/* The worst-case PRD chain of a 4 MB stream takes 67 of the 96 entries below the scratch */
#define WMT_JDEC_MAX_STREAM	SZ_4M
...
/* CAPTURE is the MCU-padded picture; the visible size is the compose rectangle */
				   ...
				   struct v4l2_jpeg_header *hdr)
{
	...
	size_t eoi, cut, total, scratch_len, stream_off;
	...
	/* The ECS chains from a 32-byte aligned offset; all before it is copied to scratch */
	memcpy(scratch, src, 2);
	memset(scratch + 2, 0xff, WMT_JDEC_FILL_LEN);
	wmt_jdec_write_dht(scratch + WMT_JDEC_PRE_LEN);

	stream_off = min(ALIGN(hdr->ecs_offset, WMT_JDEC_PRD_ALIGN), eoi);
	total = WMT_JDEC_FILL_LEN + WMT_JDEC_DHT_LEN + stream_off;
	if (total > WMT_JDEC_TAIL_OFF - WMT_JDEC_SCRATCH_OFF)
		return -EINVAL;
	memcpy(scratch + WMT_JDEC_PRE_LEN + WMT_JDEC_DHT_LEN, src + 2, stream_off - 2);
	scratch_len = ALIGN(total, WMT_JDEC_PRD_ALIGN);
	memset(scratch + total, 0xff, scratch_len - total);

	cut = max(round_down(eoi, WMT_JDEC_PRD_ALIGN), stream_off);
	memcpy(tail, src + cut, eoi - cut);
	prd = wmt_jdec_prd_chain(prd, ctx->prd_dma + WMT_JDEC_SCRATCH_OFF, scratch_len, false);
...
/* Return buffers of current job and flag drain end */
static void wmt_jdec_complete(struct wmt_jdec_ctx *ctx, enum vb2_buffer_state state)
{
	...
	/* DECODER_CMD updates the drain state in two stores under irq_lock */
	spin_lock_irqsave(&ctx->jdec->irq_lock, flags);
	...
	spin_unlock_irqrestore(&ctx->jdec->irq_lock, flags);
...
static int wmt_jdec_s_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	/* CAPTURE buffers are sized from OUTPUT, so both queues must be idle */
	if (vb2_is_busy(v4l2_m2m_get_vq(m2m_ctx, f->type)) ||
	    (V4L2_TYPE_IS_OUTPUT(f->type) &&
	     vb2_is_busy(v4l2_m2m_get_vq(m2m_ctx, V4L2_BUF_TYPE_VIDEO_CAPTURE))))
		return -EBUSY;
...
/* Wrapper that posts EOS deterministically on STOP */
static int wmt_jdec_decoder_cmd(struct file *file, void *fh, struct v4l2_decoder_cmd *cmd)
{
	...
	/* A drain still in flight posts its EOS on completion */
	if (cmd->cmd == V4L2_DEC_CMD_STOP && stopped)
		v4l2_event_queue_fh(&ctx->fh, &wmt_jdec_eos);
