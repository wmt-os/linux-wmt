...
static unsigned short wm8505_ac97_read(struct snd_ac97 *ac97, unsigned short reg)
{
	...
	if (ret) {
		dev_err(priv->dev, "read timeout waiting for CRDY\n");
	...
	if (ret || (val & ACSR_CST)) {
		dev_err(priv->dev, "read command failed (CST or timeout)\n");
...
static void wm8505_ac97_write(struct snd_ac97 *ac97, unsigned short reg, unsigned short data)
{
	...
	ret = readl_poll_timeout_atomic(priv->regs + WM8505_ACSR, val,
					val & ACSR_CRDY, AC97_POLL_US, AC97_TIMEOUT_US);
	if (ret) {
		dev_err(priv->dev, "write timeout waiting for CRDY\n");
		mutex_unlock(&priv->lock);
		return;
	}
	...
	if (ret)
		dev_err(priv->dev, "write command timeout (CWD)\n");
	else
...
static int wm8505_ac97_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(clk))
		return dev_err_probe(&pdev->dev, PTR_ERR(clk), "failed to get AC97 clock\n");
	...
	if (IS_ERR(pinmux))
		return dev_err_probe(&pdev->dev, PTR_ERR(pinmux), "failed to get pinmux syscon\n");
